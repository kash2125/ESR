...
PROJECTS_DB_PATH = Path(os.environ.get("PROJECTS_DB_PATH", str(BASE_DIR / "projects.db")))
...
DEFAULT_DESCRIPTION_COLOR = "#000000"
ALLOWED_DESCRIPTION_COLORS = {"#000000", "#c62828"}
...
def _is_valid_sales_person_choice(value: str) -> bool:
    return (value or "").strip() in SALES_PERSON_OPTIONS


def _is_valid_project_status_choice(value: str) -> bool:
    return (value or "").strip() in PROJECT_STATUS_OPTIONS
...
def _normalize_description_color(value: str) -> str:
    raw = (value or "").strip().lower()
    if raw in {"black", "rgb(0,0,0)", "rgb(0, 0, 0)"}:
        raw = "#000000"
    if raw in {"red", "rgb(198,40,40)", "rgb(198, 40, 40)", "rgb(255,0,0)", "rgb(255, 0, 0)"}:
        raw = "#c62828"
    if raw in ALLOWED_DESCRIPTION_COLORS:
        return raw
    return DEFAULT_DESCRIPTION_COLOR


def _is_valid_description_color(value: str) -> bool:
    return _normalize_description_color(value) in ALLOWED_DESCRIPTION_COLORS


class _DescriptionHTMLSanitizer(HTMLParser):
    _allowed_tags = {"span", "strong", "b", "em", "i", "u", "br", "div", "p", "ul", "ol", "li"}
    _void_tags = {"br"}
    ...
    def handle_starttag(self, tag: str, attrs):
        ...
        if tag == "span":
            ...
            for key, value in attrs:
                if (key or "").lower() != "style":
                    continue
                match = re.search(r"color\s*:\s*(#[0-9A-Fa-f]{6})", value or "")
                if match:
                    color = match.group(1).lower()
                    break
            normalized_color = _normalize_description_color(color or "")
            if normalized_color in ALLOWED_DESCRIPTION_COLORS:
                self.parts.append(f'<span style="color: {normalized_color};">')
            else:
    ...
    def handle_data(self, data: str):
        if not data:
            return
        self.parts.append(html_escape(data).replace("\r\n", "\n").replace("\r", "\n").replace("\n", "<br>"))
...
def _description_text_content(value: str) -> str:
    raw = (value or "").strip()
    if not raw:
        return ""
    no_tags = re.sub(r"<[^>]+>", " ", raw)
    plain = html_unescape(no_tags)
...
def _sanitize_description_html(value: str) -> str:
    ...
    try:
        ...
        return html_escape(raw).replace("\r\n", "\n").replace("\r", "\n").replace("\n", "<br>")
    sanitized = "".join(sanitizer.parts).strip()
    sanitized = re.sub(r"(?:<br>){3,}", "<br><br>", sanitized)
    return sanitized


def _build_description_html(description: str) -> str:
    sanitized = _sanitize_description_html(description)
    return sanitized or "No description added."
    ...
    with _db_conn() as conn:
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS projects (
                ...
                description_color TEXT NOT NULL DEFAULT '#24324b',
                created_at TEXT NOT NULL
            )
            """
        )
        columns = {row[1] for row in conn.execute("PRAGMA table_info(projects)").fetchall()}
        if "description_color" not in columns:
            conn.execute("ALTER TABLE projects ADD COLUMN description_color TEXT NOT NULL DEFAULT '#24324b'")
        conn.commit()


def _migrate_json_projects_if_needed() -> None:
    _init_projects_db()
    try:
        with _db_conn() as conn:
    ...
    for row in payload:
        ...
        cleaned_rows.append({
            ...
            "description_color": _normalize_description_color(str(row.get("description_color", DEFAULT_DESCRIPTION_COLOR))),
            "description_html": _build_description_html(str(row.get("description", "")).strip()),
            "created_at": str(row.get("created_at", "")).strip() or datetime.now(APP_TZ).isoformat(),
        })
        ...
        with _db_conn() as conn:
            conn.executemany(
                ...
                INSERT OR REPLACE INTO projects (
                    project_number, project_name, engineer, project_status,
                    sales_person, start_date, projected_end_date, description, description_color, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                [
                    (
                        row["project_number"], row["project_name"], row["engineer"], row["project_status"],
                        row["sales_person"], row["start_date"], row["projected_end_date"], row["description"], row["description_color"], row["created_at"]
                    )
...
def _load_projects() -> list[dict]:
    _migrate_json_projects_if_needed()
    try:
        with _db_conn() as conn:
            rows = conn.execute(
                """
                SELECT project_number, project_name, engineer, project_status,
                       sales_person, start_date, projected_end_date, description, description_color, created_at
                FROM projects
                ORDER BY datetime(created_at) DESC, project_number DESC
                """
            ).fetchall()
    except Exception:
        return []
    ...
    for row in rows:
        cleaned.append({
            "project_number": str(row["project_number"] or "").strip(),
            "project_name": str(row["project_name"] or "").strip(),
            "engineer": _normalize_engineer(str(row["engineer"] or "")),
            "project_status": _normalize_project_status(str(row["project_status"] or "")),
            "sales_person": _normalize_sales_person(str(row["sales_person"] or "")),
            "start_date": str(row["start_date"] or "").strip(),
            "projected_end_date": str(row["projected_end_date"] or "").strip(),
            "description": str(row["description"] or "").strip(),
            "description_color": _normalize_description_color(str(row["description_color"] or DEFAULT_DESCRIPTION_COLOR)),
            "description_html": _build_description_html(str(row["description"] or "").strip()),
...
def _save_projects(projects: list[dict]) -> None:
    ...
    with _db_conn() as conn:
        ...
        conn.executemany(
            ...
            [
                (
                    str(project.get("project_number", "")).strip(),
                    str(project.get("project_name", "")).strip(),
                    _normalize_engineer(str(project.get("engineer", ""))),
                    _normalize_project_status(str(project.get("project_status", ""))),
                    _normalize_sales_person(str(project.get("sales_person", ""))),
                    str(project.get("start_date", "")).strip(),
                    str(project.get("projected_end_date", "")).strip(),
                    str(project.get("description", "")).strip(),
                    _normalize_description_color(str(project.get("description_color", DEFAULT_DESCRIPTION_COLOR))),
                    str(project.get("created_at", "")).strip() or datetime.now(APP_TZ).isoformat(),
                )
                for project in projects
                if str(project.get("project_number", "")).strip() and str(project.get("project_name", "")).strip()
            ],
        )
        conn.commit()


def _insert_project(project: dict) -> None:
    _init_projects_db()
    with _db_conn() as conn:
        conn.execute(
            """
...
def _get_project_by_number(project_number: str) -> dict | None:
    _init_projects_db()
    with _db_conn() as conn:
        row = conn.execute(
            """
            SELECT project_number, project_name, engineer, project_status,
                   sales_person, start_date, projected_end_date, description, description_color, created_at
            FROM projects
            WHERE lower(project_number) = lower(?)
            LIMIT 1
            """,
            ((project_number or "").strip(),),
        ).fetchone()
...
def _update_project(original_project_number: str, project: dict) -> bool:
    _init_projects_db()
    with _db_conn() as conn:
        cursor = conn.execute(
            """
            ...
            (
                ...
                _normalize_description_color(str(project.get("description_color", DEFAULT_DESCRIPTION_COLOR))),
                str(project.get("created_at", "")).strip() or datetime.now(APP_TZ).isoformat(),
                (original_project_number or "").strip(),
            ),
        )
        conn.commit()
...
def _delete_project_by_number(project_number: str) -> bool:
    _init_projects_db()
    with _db_conn() as conn:
        cursor = conn.execute(
            "DELETE FROM projects WHERE lower(project_number) = lower(?)",
            ((project_number or "").strip(),),
        )
        conn.commit()
        return cursor.rowcount > 0
...
def _build_timeline_axis_ticks(start_iso: str | None, end_iso: str | None) -> list[dict]:
    start_dt = _parse_iso_date(start_iso or "")
    end_dt = _parse_iso_date(end_iso or "")
    ...
    first_tick = date(start_dt.year, start_dt.month, 1)
    last_tick = date(end_dt.year, end_dt.month, 1)
    total_months = max(((last_tick.year - first_tick.year) * 12) + (last_tick.month - first_tick.month), 0)
    if total_months <= 12:
        step_months = 1
    ...
    while cursor <= last_tick:
        left_pct = ((cursor - start_dt).days / total_days) * 100
        ticks.append({
            "label": cursor.strftime("%b %Y"),
            "left_pct": round(min(max(left_pct, 0), 100), 2),
        })
        cursor = _add_months(cursor, step_months)

    end_label = end_dt.strftime("%b %Y")
    if not ticks or ticks[-1]["label"] != end_label:
        ticks.append({
            "label": end_label,
            "left_pct": 100.0,
        })
...
def _dashboard_data(projects: list[dict]) -> dict:
    ...
    timeline_axis_ticks = _build_timeline_axis_ticks(timeline_start, timeline_end)
    ...
    return {
        ...
        "timeline_axis_ticks": timeline_axis_ticks,
    }
...
def index():
    ...
    return render_template(
        ...
        timeline_axis_ticks=dashboard["timeline_axis_ticks"],
        esr_link=esr_link,
...
def _redirect_to_index_with_state(engineer: str = "", sort: str = ""):
    params = {}
    if (engineer or "").strip():
        params["engineer"] = engineer.strip()
    if (sort or "").strip():
        params["sort"] = sort.strip()
    return redirect(url_for("index", **params))


def _project_form_context(form_values: dict | None = None) -> dict:
    ...
    return {
        ...
        "engineer": values.get("engineer", ""),
        "project_status": values.get("project_status", "No Contract"),
        "sales_person": values.get("sales_person", ""),
        "start_date": values.get("start_date", ""),
        "projected_end_date": values.get("projected_end_date", ""),
        "description": values.get("description", ""),
        "description_html": _build_description_html(values.get("description", "")),
        "description_color": values.get("description_color", DEFAULT_DESCRIPTION_COLOR),
        "return_engineer": values.get("return_engineer", ""),
        "return_sort": values.get("return_sort", ""),
    }
...
def _render_project_form(
    *,
    ...
    page_title: str = "Add Project",
    intro_text: str | None = None,
    submit_label: str = "Save Project",
    form_action: str | None = None,
    original_project_number: str = "",
    editing: bool = False,
    return_engineer: str = "",
    return_sort: str = "",
):
    context = _project_form_context(form_values)
    context.update({
        "page_title": page_title,
        "intro_text": intro_text or "Fill in the project details below. Once submitted, the dashboard updates automatically.",
        "submit_label": submit_label,
        "form_action": form_action or url_for("add_project"),
        "original_project_number": original_project_number,
        "editing": editing,
        "return_engineer": return_engineer,
        "return_sort": return_sort,
    })
    return render_template("add_project.html", **context)


def _project_form_values_from_request(form) -> dict:
    return {
        "project_number": (form.get("project_number") or "").strip(),
        "project_name": (form.get("project_name") or "").strip(),
        "engineer": (form.get("engineer") or "").strip(),
        "project_status": (form.get("project_status") or "").strip() or "No Contract",
        "sales_person": (form.get("sales_person") or "").strip(),
        "start_date": (form.get("start_date") or "").strip(),
        "projected_end_date": (form.get("projected_end_date") or "").strip(),
        "description": (form.get("description") or "").strip(),
        "description_color": (form.get("description_color") or "").strip() or DEFAULT_DESCRIPTION_COLOR,
        "return_engineer": (form.get("return_engineer") or "").strip(),
        "return_sort": (form.get("return_sort") or "").strip(),
    }


def _prepare_project_payload(form_values: dict, *, created_at: str | None = None) -> tuple[dict | None, str | None]:
    project_number = (form_values.get("project_number") or "").strip()
    project_name = (form_values.get("project_name") or "").strip()
    raw_engineer = (form_values.get("engineer") or "").strip()
    raw_project_status = (form_values.get("project_status") or "").strip()
    raw_sales_person = (form_values.get("sales_person") or "").strip()
    start_date = (form_values.get("start_date") or "").strip()
    projected_end_date = (form_values.get("projected_end_date") or "").strip()
    description = (form_values.get("description") or "").strip()
    description_color = (form_values.get("description_color") or DEFAULT_DESCRIPTION_COLOR).strip()
    description_html = _sanitize_description_html(description)

    if not all([project_number, project_name, raw_engineer, raw_project_status, raw_sales_person, start_date, projected_end_date]):
        return None, "Please fill in all project fields before saving."

    if not _description_text_content(description_html):
        return None, "Please add a description before saving."

    if not _is_valid_engineer_choice(raw_engineer):
        return None, "Please select a valid Project Engineer from the dropdown."

    if not _is_valid_project_status_choice(raw_project_status):
        return None, "Please select a valid Project Status from the dropdown."

    if not _is_valid_sales_person_choice(raw_sales_person):
        return None, "Please select a valid Sales Person from the dropdown."

    if not _is_valid_description_color(description_color):
        return None, "Please select a valid Description Font Color."

    start_dt = _parse_iso_date(start_date)
    end_dt = _parse_iso_date(projected_end_date)
    if not start_dt or not end_dt:
        return None, "Please select valid start and end dates."

    if end_dt < start_dt:
        return None, "Projected End Date cannot be earlier than Start Date."
    ...
    payload = {
        ...
        "description_color": _normalize_description_color(description_color),
        "created_at": (created_at or "").strip() or datetime.now(APP_TZ).isoformat(),
    }
    return payload, None
...
def add_project():
    ...
    return_engineer = (request.values.get("return_engineer") or request.args.get("engineer") or "").strip()
    return_sort = (request.values.get("return_sort") or request.args.get("sort") or "").strip()

    if request.method == "POST":
        form_values = _project_form_values_from_request(request.form)
        payload, error = _prepare_project_payload(form_values)
        if error:
            flash(error)
            return _render_project_form(form_values=form_values)

        duplicate = _get_project_by_number(payload["project_number"])
        if duplicate:
            flash("That project number already exists. Use a different project number.")
            return _render_project_form(form_values=form_values)

        try:
            _insert_project(payload)
        except sqlite3.IntegrityError:
            flash("That project number already exists. Use a different project number.")
            return _render_project_form(form_values=form_values)

        flash("Project added successfully.")
        return _redirect_to_index_with_state(form_values.get("return_engineer", ""), form_values.get("return_sort", ""))

    return _render_project_form(return_engineer=(request.args.get("engineer") or "").strip(), return_sort=(request.args.get("sort") or "").strip())


@app.route("/edit-project/<project_number>", methods=["GET", "POST"])
def edit_project(project_number: str):
    if not _is_project_admin_logged_in():
        return redirect(url_for("project_admin_login", next=url_for("edit_project", project_number=project_number)))

    existing_project = _get_project_by_number(project_number)
    ...
    if request.method == "POST":
        original_project_number = (request.form.get("original_project_number") or project_number).strip()
        original_project = _get_project_by_number(original_project_number)
        if not original_project:
            flash("Project not found.")
            return redirect(url_for("index"))

        form_values = _project_form_values_from_request(request.form)
        payload, error = _prepare_project_payload(form_values, created_at=original_project.get("created_at", ""))
        if error:
            flash(error)
            return _render_project_form(
                form_values=form_values,
                page_title="Edit Project",
                intro_text="Update the project details below. Saving will immediately refresh the dashboard.",
                submit_label="Update Project",
                form_action=url_for("edit_project", project_number=project_number),
                original_project_number=original_project_number,
                editing=True,
                return_engineer=return_engineer,
                return_sort=return_sort,
        ...
        duplicate = _get_project_by_number(payload["project_number"])
        if duplicate and duplicate.get("project_number", "").lower() != original_project_number.lower():
            flash("That project number already exists. Use a different project number.")
            return _render_project_form(
                form_values=form_values,
                page_title="Edit Project",
        ...
        try:
            updated = _update_project(original_project_number, payload)
        except sqlite3.IntegrityError:
            flash("That project number already exists. Use a different project number.")
            return _render_project_form(
                form_values=form_values,
        ...
        if not updated:
            flash("Project not found. Nothing was updated.")
        else:
            flash(f"Project {payload['project_number']} updated.")
        return _redirect_to_index_with_state(return_engineer, return_sort)

    return _render_project_form(
        form_values=existing_project,
        page_title="Edit Project",
        intro_text="Update the project details below. Saving will immediately refresh the dashboard.",
        submit_label="Update Project",
        form_action=url_for("edit_project", project_number=project_number),
        original_project_number=existing_project.get("project_number", ""),
        editing=True,
        return_engineer=return_engineer,
        return_sort=return_sort,
    )
...
def delete_project(project_number: str):
    ...
    deleted = _delete_project_by_number(project_number)
    ...
    return _redirect_to_index_with_state((request.form.get("return_engineer") or "").strip(), (request.form.get("return_sort") or "").strip())
