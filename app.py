...
def _project_chart_data(projects: list[dict]) -> dict[str, list]:
    counts = Counter((p.get("engineer") or "Unassigned") for p in projects)
...
def index():
    ...
    chart = _project_chart_data(projects)
    esr_link = url_for("esr_index")
    return render_template(
        "dashboard.html",
        projects=projects,
        total_projects=len(projects),
        total_engineers=len(chart["labels"]),
...
def add_project():
    ...
    if request.method == "POST":
        ...
        if not project_number or not project_name or not engineer:
            flash("Please fill in Project Number, Project Name, and Engineer.")
