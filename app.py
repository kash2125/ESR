...
# Auth config (override via environment variables)
...
# TEMPORARY: Disable password authentication gate.
# Set AUTH_ENABLED=1 (or true/yes) in the environment to re-enable later (same credentials).
AUTH_ENABLED = os.environ.get("AUTH_ENABLED", "0").strip().lower() in {"1", "true", "yes", "y"}


@app.before_request
def _require_login_for_everything():
    # Auth gate is optional. When disabled, the app behaves as if already authenticated.
    if not AUTH_ENABLED:
        return None

    # Allow the login/logout endpoints without an active session.
    public_endpoints = {"login", "login_post", "logout", "static"}
    ...
    # JSON endpoints should return 401 (no HTML redirect surprise for fetch()).
