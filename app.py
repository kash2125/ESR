...
def _load_projects() -> list[dict]:
    ...
    for row in payload:
        ...
        cleaned.append({
            ...
            "engineer": str(row.get("engineer", "")).strip() or "Unassigned",
...
def _build_timeline_rows(projects: list[dict]) -> tuple[list[dict], str | None, str | None]:
    rows: list[dict] = []
    ...
    overall_start = min(start for start, _ in parsed_ranges)
    overall_end = max(end for _, end in parsed_ranges)
    ...
    return rows, overall_start.isoformat(), overall_end.isoformat()
...
def _dashboard_data(projects: list[dict]) -> dict:
    ...
    timeline_rows, timeline_start, timeline_end = _build_timeline_rows(projects)
...
def add_project():
    ...
    if request.method == "POST":
        ...
        engineer = (request.form.get("engineer") or "").strip()
        ...
        if not all([project_number, project_name, engineer, raw_project_status, raw_sales_person, start_date, projected_end_date, description]):
