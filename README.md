...
Authentication code is included but **disabled by default**.

Enable it:

- Windows (cmd): `set AUTH_ENABLED=1`
- PowerShell: `$env:AUTH_ENABLED="1"`

Disable it:

- Windows (cmd): `set AUTH_ENABLED=0`

When disabled, `/login` and `/logout` return 404 and the site is accessible without sign-in.
